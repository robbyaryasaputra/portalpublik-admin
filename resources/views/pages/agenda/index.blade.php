                        ...
                        <div class="input-group input-group-outline mb-0">
                            <select class="form-control" id="filter_penyelenggara" name="filter_penyelenggara">
                                ...
                                @foreach ($penyelenggaraList as $p)
                                    <option value="{{ $p }}" {{ request('filter_penyelenggara') == $p ? 'selected' : '' }}>
                                        {{ $p }}
                                    </option>
                                @endforeach
                            </select>
            ...
            <table class="table align-items-center mb-0">
              <thead>
                <tr>
                  ...
                  <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Judul Agenda</th>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="text-center">
                    ...
                    @else
                        ...
                        @php
                            ...
                            $colors = [
                                'bg-gradient-primary', 'bg-gradient-success', 'bg-gradient-info', 
                                'bg-gradient-danger', 'bg-gradient-warning', 'bg-gradient-dark'
                            ];
                  ...
                  {{-- Data Agenda --}}
                  <td>
                      <h6 class="mb-0 text-sm text-wrap" style="max-width: 250px;">{{ $agenda->judul }}</h6>
                  </td>
                  <td>
                    <p class="text-xs font-weight-bold mb-0">{{ $agenda->tanggal_mulai->format('d M Y') }}</p>
                    <p class="text-xs text-secondary mb-0">{{ $agenda->tanggal_mulai->format('H:i') }} WIB</p>
                  </td>
                  <td>
                      <p class="text-xs font-weight-bold mb-0 text-wrap" style="max-width: 150px;">{{ $agenda->lokasi ?? '-' }}</p>
                  </td>
                  <td>
